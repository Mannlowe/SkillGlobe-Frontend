...
export default function CertificateForm({
  ...
  const filteredCertificateEntries = certificateEntries.filter(
    (entry) =>
      (entry.certificateName ? entry.certificateName.trim() !== "" : false) ||
      entry.certificateFile !== null
  );
  ...
  const removeCertificate = (entryId: string) => {
    ...
    if (activeEntryId === entryId) {
      const remainingEntries = certificateEntries.filter(
  ...
  const validateActiveEntry = () => {
    const activeEntry = certificateEntries.find(
    ...
    if (validateActiveEntry()) {
        ...
        setCertificateEntries((prev) =>
          prev.map((entry) =>
            entry.id === activeEntryId
              ? { ...entry, uploadError: "Authentication failed" }
              : entry
          )
        );
        return;
      ...
      try {
        ...
        setCertificateEntries((prev) =>
          prev.map((entry) =>
            entry.id === activeEntryId
              ? { ...entry, isUploading: true, uploadError: undefined }
              : entry
          )
        );
  ...
  const deleteApiCertificate = async (cert: any) => {
    ...
    if (
      confirm(
        `Are you sure you want to delete certificate: ${cert.document_name}?`
      )
    ) {
  ...
  const SortableApiCertificateItem = ({
    id,
    cert,
  }: SortableApiCertificateItemProps) => {
    const {
      attributes,
      listeners,
      setNodeRef,
        ...
        <div className="flex items-center w-full">
          ...
          <div className="flex-grow">
            <h4 className="font-medium text-gray-900">
              {cert.document_name || "Untitled Certificate"}
            </h4>
            <p className="text-sm text-gray-600">
              {cert.document_upload || "No file information"}
            </p>
          </div>
    ...
    return (
      ...
      >
        <div className="flex items-center w-full">
          ...
          <div className="flex-grow">
            <h4 className="font-medium text-gray-900">
              {entry.certificateName || "Untitled Certificate"}
            </h4>
            <p className="text-sm text-gray-600">
              {entry.certificateFileName
                ? `File: ${entry.certificateFileName}`
                : "No file uploaded"}
            </p>
          </div>
          ...
          <div>
            {certificateEntries.length > 0 && (
              <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200">
                <h4 className="font-medium text-gray-900">
                  {certificateEntries.find((e) => e.id === activeEntryId)
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const updatedList =
                          response.message?.data?.certificate_list || [];
